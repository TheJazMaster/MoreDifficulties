...
internal static class CharacterPatches
{
	...
	private static void Character_Render_Postfix(Character __instance, G g, int x, int y, bool flipX, bool mini, bool? isSelected, bool autoFocus, UIKey rightHint, UIKey leftHint, UIKey downHint, UIKey upHint, bool renderLocked, bool canFocus, bool showTooltips, UIKey? overrideKey = null)
	{
		...
		RenderBoxes(__instance, g, x, y, flipX, mini, isSelected, autoFocus, rightHint, leftHint, downHint, upHint, runSummaryRoute, overrideKey);
		RenderTooltips(__instance, g, mini, renderLocked, canFocus, showTooltips, overrideKey);
		RenderLockAndBan(__instance, g, x, y, mini, isSelected, autoFocus, rightHint, leftHint, downHint, upHint, runSummaryRoute, overrideKey);
	}
	...
	private static void RenderBoxes(Character character, G g, int x, int y, bool flipX, bool mini, bool? isSelected, bool autoFocus, UIKey rightHint, UIKey leftHint, UIKey downHint, UIKey upHint, RunSummaryRoute? runSummaryRoute, UIKey? overrideKey)
	{
		if (!mini || skipRenderingCharacterExtras || character.deckType is not { } deck || g.state.route is not { } route || !(IsInRunOptionsScreen(g) || IsInRunSummaryScreen(g) || AreCockpitPanelsShown(g)) || !Instance.AltStarters.HasAltStarters(character.deckType.Value)) return;
	...
	private static void RenderLockAndBan(Character character, G g, int x, int y, bool mini, bool? isSelected, bool autoFocus, UIKey rightHint, UIKey leftHint, UIKey downHint, UIKey upHint, RunSummaryRoute? runSummaryRoute, UIKey? overrideKey)
	{
		var deckType = character.deckType;

		if (!mini || skipRenderingCharacterExtras || character.deckType is not { } deck || !IsInRunOptionsScreen(g) || runSummaryRoute != null) return;
	...
	private static bool IsInRunSummaryScreen(G g) {
		return g.state.route is RunSummaryRoute || g?.metaRoute?.subRoute is Codex;
	}
}
